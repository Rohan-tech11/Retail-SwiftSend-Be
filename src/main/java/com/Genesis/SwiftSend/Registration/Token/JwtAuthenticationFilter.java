...
public class JwtAuthenticationFilter extends OncePerRequestFilter {
	...
	protected void doFilterInternal(HttpServletRequest request,
		...
		if (!requestURI.startsWith("/api/register")
			...
			// ,rest controller advice,so we used http servlet response to build
			// the
			// exception
			if (jwt == null || jwt.isEmpty()) {
				...
				String jsonResponse = "{\"error\": \"" + errorResponse
			...
			// Request path is related to registration or login, skip JWT
			// verification
			filterChain.doFilter(request, response);
		}
	}
	...
	private String extractJwtFromRequest(HttpServletRequest request) {
		// Typically, JWTs are included in the "Authorization" header, like
		// "Bearer
		// <token>"
		String authorizationHeader = request.getHeader("Authorization");
		...
		// If the JWT is not found in the "Authorization" header, you may check
		// other
		// headers or request parameters
		// For example, if you use a custom header like "X-JWT-Token", you can
		// check it
		// here

		// If not found in any header, you may check request parameters or
		// cookies, if
		// applicable
